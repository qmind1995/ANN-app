...
namespace parameters{
    ...
    static const int SINGLESAMPLE = 20;
    static const int BATCH = 21;
    struct paramHolder {
        NeuralNetwork* net;
        Trainer * trainer;
...
inline static vector<int> voteOuput(vector<DataEntry*> data){

    int numLabels = data[0]->target.n_rows;
    vector<int> vote(numLabels,0);
    for(int i=0; i< data.size(); i++){
        for(int v =0; v<numLabels; v++){
            if(data[i]->target(v,0) == 1){
                vote[v] ++;
            }
        }
    }
